...
var Canvas = class {
  ...
  drawParticle(particle, delta) {
    ...
    this.draw((ctx) => {
      var _a;
      const container = this.container, options2 = container.actualOptions, zIndexOptions = particle.options.zIndex, zIndexFactor = zIndexFactorOffset - particle.zIndexFactor, zOpacityFactor = zIndexFactor ** zIndexOptions.opacityRate, opacity = particle.bubble.opacity ?? ((_a = particle.opacity) == null ? void 0 : _a.value) ?? defaultOpacity, strokeOpacity = particle.strokeOpacity ?? opacity, zOpacity = opacity * zOpacityFactor, zStrokeOpacity = strokeOpacity * zOpacityFactor, transform = {}, colorStyles = {
...
var Engine = class {
  ...
  async load(params) {
    var _a;
    const id = params.id ?? ((_a = params.element) == null ? void 0 : _a.id) ?? `tsparticles${Math.floor(getRandom() * loadRandomFactor)}`, { index, url } = params, options2 = url ? await getDataFromUrl({ fallback: params.options, url, index }) : params.options;
  ...
  loadOptions(options2, sourceOptions) {
    this.plugins.forEach((plugin) => {
      var _a;
      return (_a = plugin.loadOptions) == null ? void 0 : _a.call(plugin, options2, sourceOptions);
    });
  }
  loadParticlesOptions(container, options2, ...sourceOptions) {
    ...
    updaters.forEach((updater) => {
      var _a;
      return (_a = updater.loadOptions) == null ? void 0 : _a.call(updater, options2, ...sourceOptions);
    });
  }
...
var CircleWarp = class extends Circle {
  ...
  intersects(range) {
    ...
    if (circle.radius !== void 0) {
      const biggerCircle = new Circle(newPos.x, newPos.y, circle.radius * double4);
      return super.intersects(biggerCircle);
    } else if (rect.size !== void 0) {
      const rectSW = new Rectangle(newPos.x, newPos.y, rect.size.width * double4, rect.size.height * double4);
...
var LinkInstance = class {
  constructor(container, engine) {
    this._drawLinkLine = (p1, link) => {
      ...
      const container2 = this._container, options2 = container2.actualOptions, p2 = link.destination, pos1 = p1.getPosition(), pos2 = p2.getPosition();
      ...
      container2.canvas.draw((ctx) => {
        ...
        const width = p1.retina.linksWidth ?? minWidth, maxDistance = p1.retina.linksDistance ?? minDistance3, { backgroundMask } = options2;
        drawLinkLine({
          context: ctx,
          width,
          begin: pos1,
    ...
    this._drawLinkTriangle = (p1, link1, link2) => {
      ...
      const container2 = this._container, options2 = container2.actualOptions, p2 = link1.destination, p3 = link2.destination, opacityTriangle = triangleOptions.opacity ?? (link1.opacity + link2.opacity) * half3;
  ...
  drawParticle(context, particle) {
    ...
    const p1Links = links.filter((l) => options2.links && (options2.links.frequency >= maxFrequency || this._getLinkFrequency(particle, l.destination) <= options2.links.frequency));
